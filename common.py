...
class outlineBaseCommand:
    """
    Convenience functions for outline TextCommands
    """
    @property
    def path(self):
        return self.view.settings().get('outline_path')
    ...
    def filecount(self):
        ...
        """
        return self.view.settings().get('outline_count', 0)
    ...
    def show_parent(self):
        return self.view.settings().get('outline_show_parent', False)
    ...
    def get_parent(self, line, path):
        ...
        Returns relative path for line
            • line is a region
            • path is self.path
            • self.index is list stored in view settings as 'outline_index'
        '''
    ...
    def get_all(self):
        ...
        outline_index is always supposed to represent current state of view,
    ...
    def _mark(self, mark, regions):
        ...
        if marked:
            ...
            self.view.add_regions('marked', r, 'outline.marked', '', MARK_OPTIONS)
        else:
    ...
    def set_ui_in_rename_mode(self, edit):
        header = self.view.settings().get('outline_header', False)
        if header:
            regions = self.view.find_by_selector('text.outline header.outline punctuation.definition.separator.outline')
        else:
            regions = self.view.find_by_selector('text.outline outline.item.parent_dir')
        if not regions:
            return
        region = regions[0]
        start = region.begin()
